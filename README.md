...
1. Copy the sample configuration and adjust paths/weights:
  ...
  # edit config.yaml to point at your data, tweak tagging settings, and set defaults
  ```
...
- `scripts/confidence_insights.ipynb` — highlights the top 25 contacts from `confidence_report.csv` in the `very_high` and `high` buckets, including source counts and available contact channels (with invalid/non-standard details surfaced).
...
- `scripts/invalid_email_insights.ipynb` — lists every consolidated contact with an invalid email that was dropped during normalization.
- `scripts/non_standard_phone_insights.ipynb` — lists contacts holding phone numbers that failed strict normalization so they can be audited.
...
- Logging defaults to `WARNING`. Override globally via `config.yaml` (`logging.level`), per-run via `--log-level`, or ad hoc with the `CONTACTS_ETL_LOG_LEVEL` environment variable.
