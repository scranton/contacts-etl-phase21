...
def confidence_score(row, vmap: Dict[str, Dict[str, int]]) -> int:
    ...
    if email_valid: score += 5
    if phone_valid: score += 3
...
def main():
    ...
    bins = []
    for s in scores:
        if s >= 80: bins.append("very_high")
        elif s >= 60: bins.append("high")
        elif s >= 40: bins.append("medium")
        else: bins.append("low")
    ...
    summary = {
        ...
        "bucket_pct": contacts_df["confidence_bucket"].value_counts(normalize=True).mul(100).round(2).to_dict()
    }
    # Save summary CSV
    rows = []
