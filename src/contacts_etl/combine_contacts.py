...
# configure basic logging near module top
...
# Minimal but effective starter set (expand as needed)
_NICKMAP = {
...
def clean_email_val(e: Optional[str]) -> str:
    ...
    if HAS_EMAIL_VALIDATOR:
        try:
            return email_validator.validate_email(e, check_deliverability=False).normalized
...
def normalize_phone(num: Any, default_country: str = "US") -> str:
    if not num or str(num).strip() == "": return ""
    raw = str(num).strip()
    if HAS_PHONENUMBERS:
        try:
    ...
    if raw.startswith("+"): return re.sub(r"[^\d+]", "", raw)
...
def load_linkedin_csv(path: Optional[str]) -> List[Dict[str, Any]]:
    if not path or not os.path.exists(path):
        logging.warning("Linkedin path missing: %s", path)
        return []
    with open(path, "r", encoding="utf-8", errors="ignore") as fh:
        txt = fh.read().splitlines()
    header_idx = None
    for i, line in enumerate(txt[:100]):
        if line.strip().startswith("First Name,Last Name,URL"):
            header_idx = i; break
    ...
    for i, row in df.iterrows():
        ...
        rec: Dict[str, Any] = {
            ...
            "linkedin_url": str(row.get("URL","")).strip() if "linkedin.com" in str(row.get("URL","")).lower() else "",
            "source":"linkedin","source_row_id": str(i)
        }
        em = str(row.get("Email Address","")).strip()
        if em: rec["emails"].append({"value": em, "label": "work"})

        # Skip records with no usable information
...
def load_gmail_csv(path: Optional[str]) -> List[Dict[str, Any]]:
    if not path or not os.path.exists(path):
        logging.warning("GMail path missing: %s", path)
        return []
    df = pd.read_csv(path, dtype=str, keep_default_na=False)
    rows : List[Dict[str, Any]] = []
    for i, row in df.iterrows():
        ...
        rec: Dict[str, Any] = {
            "full_name_raw": raw_full, "full_name":"",
            "first_name":"","middle_name":"","last_name":"",
            "maiden_name":"", "suffix": str(row.get("Name Suffix","")).strip(), "suffix_professional":"",
            "emails":[], "phones":[], "addresses":[],
            "company": str(row.get("Organization Name","")).strip(),
            "title": str(row.get("Organization Title","")).strip(),
            "linkedin_url":"", "source":"gmail", "source_row_id": str(i)
        }
        for n in range(1,5):
            v2 = str(row.get(f"E-mail {n} - Value", "")).strip()
            if v2: rec["emails"].append({"value": v2, "label": str(row.get(f"E-mail {n} - Type", "")).strip().lower()})
        for n in range(1,5):
            v2 = str(row.get(f"Phone {n} - Value", "")).strip()
            if v2: rec["phones"].append({"value": v2, "label": str(row.get(f"Phone {n} - Label", "")).strip().lower()})
        for n in range(1,4):
            addr = {
                "po_box": str(row.get(f"Address { n } - PO Box","")).strip(),
                "extended": "",
                "street": str(row.get(f"Address { n } - Street","")).strip() or str(row.get(f"Address { n } - Formatted","")).strip(),
                "city": str(row.get(f"Address { n } - City","")).strip(),
                "state": str(row.get(f"Address { n } - Region","")).strip(),
                "postal_code": str(row.get(f"Address { n } - Postal Code","")).strip(),
                "country": str(row.get(f"Address { n } - Country","")).strip(),
                "label": str(row.get(f"Address { n } - Label","")).strip()
            }
...
def load_vcards(path: Optional[str]) -> List[Dict[str, Any]]:
    if not path or not os.path.exists(path):
        logging.warning("VCF path missing: %s", path)
        return []
    with open(path, "r", encoding="utf-8", errors="ignore") as fh:
        content = fh.read()
    rows: List[Dict[str, Any]] = []
...
def choose_best_first_name(records: List[Tuple[int, Dict[str, Any]]]) -> Tuple[str, str]:
    ...
    keys = list(counts.keys()); merged = {}
    for k in keys:
        if k in merged: continue
        merged[k] = counts[k]
        for j in keys:
            if j == k or j in merged: continue
            if edit_distance_leq1(k, j):
                merged[k] += counts[j]; merged[j] = -1
...
def build(args: Namespace) -> Tuple[DataFrame, DataFrame]:
    ...
    for r in all_rows:
        ...
        seen=set(); uniq=[]
        for a in addrs:
            key=(a.get("po_box",""),a.get("extended",""),a.get("street",""),a.get("city",""),a.get("state",""),a.get("postal_code",""),a.get("country",""))
            if key not in seen:
    ...
    # Merge clusters
    merged_contacts=[]; lineage_records=[]; duplicate_merge_count=0
    for root1, records in clusters.items():
        ...
        def uniq(lst: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            seen1=set(); out=[]
            for d in lst:
                v6 = d.get("value", "")
        ...
        emails = uniq(emails)
        phones = uniq(phones)
        seen_addr=set(); uniq_addrs=[]
        for a in addresses:
            key=(a.get("po_box",""),a.get("extended",""),a.get("street",""),a.get("city",""),a.get("state",""),a.get("postal_code",""),a.get("country",""))
            if key not in seen_addr:
                seen_addr.add(key); uniq_addrs.append(a)
        ...
        if li:
            company = li[0].get("company",""); title = li[0].get("title","")
        else:
        ...
        linkedin_url = ""
        for _, r in records:
            if r.get("linkedin_url"): linkedin_url = r["linkedin_url"]; break
...
def main() -> int:
    ...
    logging.info("Saved: %s", contacts_path)
    logging.info("Saved: %s", lineage_path)
